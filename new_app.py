...
os.environ["GOOGLE_API_KEY"] = os.environ.get("GOOGLE_API_KEY", "dummy_key")
...
with gr.Blocks(theme=gr.themes.Soft()) as app:
    ...
    in_bg.change(fn=update_cat, inputs=[in_bg], outputs=[in_cat]) # 브랜드 라벨 선택 시 브랜드 라벨에 해당되는 카테고리만 보여줌
    ...
    def size_recom_and_log(sel_product, p_map, u_info):
        ...
        # 2. DB에 저장
        log_size_to_sheet(sel_product, u_info, rec_size, reason) #sel_product, u_info, rec_size, reason
